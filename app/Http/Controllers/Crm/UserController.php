...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->get('university_id')) {
            $users->whereHas('university', function (Builder $query) use ($request) {
        ...
        if ($name = $request->name)
            $users->whereRaw("concat(users.surname,' ',users.firstname, ' ', users.patronymic) like ?", "%{$name}%")
                ->orWhereRaw("concat(users.surname,' ',users.patronymic, ' ', users.firstname) like ?", "%{$name}%")
                ->orWhereRaw("concat(users.firstname,' ',users.surname, ' ', users.patronymic) like ?", "%{$name}%")
                ->orWhereRaw("concat(users.firstname,' ',users.patronymic, ' ', users.surname) like ?", "%{$name}%")
                ->orWhereRaw("concat(users.patronymic,' ',users.firstname, ' ', users.surname) like ?", "%{$name}%")
                ->orWhereRaw("concat(users.patronymic,' ',users.surname, ' ', users.firstname) like ?", "%{$name}%");
